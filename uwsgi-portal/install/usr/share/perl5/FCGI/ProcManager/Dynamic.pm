...
sub pm_manage
{
	...
	if (!defined($self->{min_nproc})) { $self->{min_nproc} = $ENV{MIN_NPROC} || $self->n_processes(); };
	if (!defined($self->{max_nproc})) { $self->{max_nproc} = $ENV{MAX_NPROC} || 8; };
	if (!defined($self->{delta_nproc})) { $self->{delta_nproc} = $ENV{DELTA_NPROC} || 5; };
	if (!defined($self->{delta_time})) { $self->{delta_time} = $ENV{DELTA_TIME} || 5; };
