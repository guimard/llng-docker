...
sub connect {
    ...
    # Extract SSL options for Patroni API (not for DBD::Pg)
    my $patroni_ssl_verify = delete $attr->{patroni_ssl_verify}
      // $dsn_params->{patroni_ssl_verify};
    my $patroni_ssl_ca_file = delete $attr->{patroni_ssl_ca_file}
      // $dsn_params->{patroni_ssl_ca_file};
    my $patroni_ssl_cert_file = delete $attr->{patroni_ssl_cert_file}
      // $dsn_params->{patroni_ssl_cert_file};
    ...
    if ( defined $patroni_ssl_verify ) {
        ...
        # Accept 0, 1, 'no', 'yes', 'off', 'on', 'false', 'true'
        $ssl_opts{verify} =
          ( $patroni_ssl_verify =~ /^(1|yes|on|true)$/i ) ? 1 : 0;
    }
    $ssl_opts{ca_file}   = $patroni_ssl_ca_file   if $patroni_ssl_ca_file;
    $ssl_opts{cert_file} = $patroni_ssl_cert_file if $patroni_ssl_cert_file;
    $ssl_opts{key_file}  = $patroni_ssl_key_file  if $patroni_ssl_key_file;
    ...
    # Build leader DSN
    my $leader_dsn =
      DBD::Patroni::_build_dsn( $dsn, $leader->{host}, $leader->{port} );
    ...
    if ( @replicas && $patroni_lb ne 'leader_only' ) {
        ...
        if ($replica) {
            my $replica_dsn = DBD::Patroni::_build_dsn( $dsn, $replica->{host},
                $replica->{port} );
